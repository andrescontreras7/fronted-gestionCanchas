...
const CourtsDetailContainer = ({ courtsDetail, role }) => {
  ...
  const handleReservar = async ({ dia, hora_inicio, hora_fin, disponibilidad_id, anotaciones }) => {
    console.log('Reservando cancha:', courtsDetail.cancha_id, dia, hora_inicio, hora_fin, disponibilidad_id, anotaciones);
    setLoading(true);
    try {
      ...
      router.push('/user/bookings'); // Redirigir a la lista de reservas
    } catch (error) {
  ...
  const handleEliminar = async () => {
    ...
    try {
      ...
      // await deleteCourt(courtsDetail.cancha_id);
      console.log('Eliminando cancha:', courtsDetail.cancha_id);
  ...
  const handleToggleDisponibilidad = async () => {
    setLoading(true);
    try {
      // Aquí llamarías a tu server action para actualizar
      // await updateCourt(courtsDetail.cancha_id, {
      //   disponible: !courtsDetail.disponible
      // });
      
      console.log('Toggling disponibilidad para cancha:', courtsDetail.cancha_id);
      
      // Simular delay
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      alert(`Cancha marcada como ${!courtsDetail.disponible ? 'disponible' : 'no disponible'}`);
