...
import { MapPin, Users, DollarSign, Clock, Plus, Edit, Trash2 } from "lucide-react";
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            {role === 'admin' ? 'Gestiona las canchas del sistema' : 'Selecciona una cancha para reservar'}
          </p>
        </div>
      ...
      {courts.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courts.map(court => (
            <Card key={court.cancha_id || court.id} className="hover:shadow-lg transition-shadow duration-300">
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-4 border-t">
                  <div className="flex gap-2">
                    ...
                    {role === 'admin' ? (
                      <Link href={`/admin/courts/${court.cancha_id || court.id}`} className="flex-1">
                        <Button variant="outline" className="w-full">
                          Ver Detalles
                        </Button>
                      </Link>
                    ) : (
                      <Link href={`/user/courts/${court.cancha_id || court.id}`} className="flex-1">
                    ...
                    {role === 'admin' && (
                      <>
                        <ProtectedComponent permissions={PERMISSIONS.VER_CANCHAS}>
                          <Link href={`/admin/courts/${court.cancha_id || court.id}/horarios`}>
                        ...
                        <ProtectedComponent permissions={PERMISSIONS.EDITAR_CANCHAS}>
                          <Link href={`/admin/courts/${court.cancha_id || court.id}/edit`}>
                            <Button size="sm" variant="outline">
        ...
        <Card>
          <CardContent className="text-center py-12">
            <div className="space-y-4">
              ...
              <div>
                <h3 className="text-lg font-medium">No hay canchas disponibles</h3>
                <p className="text-muted-foreground">
                  {role === 'admin' 
        ...
        <DialogContent>
          ...
          <div className="py-4">
            <p className="text-sm text-muted-foreground">
              ¿Estás seguro que deseas eliminar la cancha <strong>{courtToDelete?.nombre}</strong>?
            </p>
