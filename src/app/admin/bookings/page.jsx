...
export default function AdminBookingsPage() {
  ...
  // Cargar todas las reservas con datos adicionales
  const loadBookings = async () => {
    ...
    try {
      ...
      const reservas = bookingsData.status === 'fulfilled' ? bookingsData.value : [];
      ...
      const usuariosMap = usuarios.reduce((map, user) => {
        map[user.user_id] = `${user.nombre} ${user.apellido}`.trim() || user.email || 'Usuario desconocido';
        return map;
      }, {});

      const canchasMap = canchas.reduce((map, court) => {
        map[court.cancha_id] = court.nombre || `Cancha ${court.cancha_id?.slice(-8)}`;
        return map;
      }, {});
      ...
      const enrichedData = reservas.map(booking => ({
        ...
        fecha_reserva: booking.fecha_inicio?.split('T')[0] || booking.fecha_reserva,
        hora_inicio: booking.fecha_inicio?.split('T')[1]?.slice(0, 5) || booking.hora_inicio,
        hora_fin: booking.fecha_fin?.split('T')[1]?.slice(0, 5) || booking.hora_fin,
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      'confirmada': { variant: 'default', text: 'Confirmada', class: 'bg-green-100 text-green-800' },
      'pendiente': { variant: 'secondary', text: 'Pendiente', class: 'bg-yellow-100 text-yellow-800' },
      'cancelada': { variant: 'destructive', text: 'Cancelada', class: 'bg-red-100 text-red-800' },
      'completada': { variant: 'outline', text: 'Completada', class: 'bg-blue-100 text-blue-800' },
    };
  ...
  const formatTime = (timeString) => {
    ...
    // Si ya es solo hora, devolverla como está
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Total Reservas</p>
                  ...
                  {filteredBookings.map((booking) => (
                    <TableRow key={booking.reserva_id}>
                      <TableCell className="font-medium">
                        <div className="max-w-32 truncate" title={booking.reserva_id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <div className="max-w-32 truncate" title={booking.usuario_nombre}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <div className="max-w-32 truncate" title={booking.cancha_nombre}>
                            {booking.cancha_nombre || 'Cancha desconocida'}
                          </div>
                        </div>
                      </TableCell>
                      <TableCell>{formatDate(booking.fecha_reserva || booking.fecha_inicio)}</TableCell>
                      <TableCell>
                        <div className="flex items-center gap-1">
                          <Clock className="h-4 w-4 text-gray-400" />
                          {formatTime(booking.fecha_inicio)} - {formatTime(booking.fecha_fin)}
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          <Button
                            ...
                            asChild
                          >
                            <Link href={`/admin/bookings/${booking.reserva_id}`}>
